...
def get_title(soup):
	...
	title = title[0:title.find(" - Wikipedia")]
...
def get_last_edit(soup):
	...
	* Finally, we don't want the whole text, 
	...
	date = date[date.find(start_text)+4:date.find(end_text)]
	#print(f"HAHAHAHAHAHA {date}")
	return date
...
def get_shortest_page(page_directory):
	...
	for page in page_directory:
		sho = page_directory[page]
		short = sho.wordcount
		if short < winning_count:
			winning_count = short
			winner = sho
...
def get_oldest_page(page_directory):
	...
	winner = None
	for article in page_directory:
		page=page_directory[article]
		timestamp = datetime.strptime(page.last_edit, '%d %B %Y')
...
class WikiPage:
	...
	def load_links(self, recursion_count=0, links_per_article=5):
		...
		shortlist=self.wiki_links[0:links_per_article]

		for wid in shortlist:
			if wid not in self.page_directory:
				page=WikiPage(self.page_directory,wid)
				if recursion_count>0:
					page.load_links(recursion_count=(recursion_count-1), links_per_article=links_per_article)
...
if __name__ == "__main__":
	...
	cat_page.display_tree()
	print("All loaded pages ", page_directory.keys())
	assert "Wildcat_(disambiguation)" in page_directory, "If it loaded the links 3 links deep from Cat, 'Wildcat_(disambiguation)' should be in the directory"
